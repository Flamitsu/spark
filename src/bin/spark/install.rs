...
pub fn install(skip_confirmation: bool){
    ...
    if user_confirmation { // If the user actually confirms
        let installation_route = detect_vfat(); // Detect the default installation routes
        if installation_route == None{ // If it didn't found any compatible route then:
            println!("Haven't found any FAT32 file system, mounted in /boot, /efi or /boot/efi");
            return; // Ends the installation process
        }
        dir_operations(true, installation_route); // This doesn't does anything for the moment. I have only put this line so the rust compiler does not say there is unused code. This is going to be replaced eventually 
